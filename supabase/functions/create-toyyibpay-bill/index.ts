...
serve(async (req) => {
  ...
  try {
    ...
    // Encode userId + amount in the reference so the callback is self-contained
    ...
    if (!billCode) {
      const msg = Array.isArray(json) && json[0]?.msg
        ? json[0].msg
    ...
    return new Response(JSON.stringify({ billCode, referenceNo }), {
